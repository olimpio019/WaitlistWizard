...
import { autorizacaoFotoLocacaoSchema } from '@shared/schema';
import type { AutorizacaoFotoLocacao } from '@shared/schema';
import { formatCPF, formatPhone } from '@/lib/utils';
import SignaturePad from '@/components/signature-pad';
import { generatePdfByFormType } from '@/lib/pdf-generator';
import FormWrapper from '@/components/form-wrapper';

export default function AutorizacaoFotoLocacao() {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [assinatura, setAssinatura] = useState<string | undefined>();
  
  // Define o formulário com o esquema de validação
  const form = useForm<AutorizacaoFotoLocacao>({
    resolver: zodResolver(autorizacaoFotoLocacaoSchema),
    defaultValues: {
      nomeProprietario: '',
      cpfCnpjProprietario: '',
      email: '',
  ...
  const onSubmit = async (data: AutorizacaoFotoLocacao) => {
    ...
    try {
      // Gera o PDF
      const pdfBlob = await generatePdfByFormType('autorizacao-foto-locacao', data, assinatura);
      
      // Cria um objeto FormData para enviar o formulário e o PDF
      const formData = new FormData();
      formData.append('tipoFormulario', 'autorizacao-foto-locacao');
      formData.append('formData', JSON.stringify(data));
      formData.append('arquivo', new File([pdfBlob], `autorizacao-foto-locacao-${data.nomeProprietario}.pdf`, { type: 'application/pdf' }));
  ...
  return (
    <FormWrapper title="Autorização com Foto - Locação">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Informações sobre a autorização de locação com fotos</CardDescription>
            </CardHeader>
            <CardContent className="grid gap-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                      ...
                      <div className="space-y-1 leading-none">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Autorizo a Lanza Imóveis a realizar visitas acompanhadas ao imóvel com potenciais locatários.
                        </p>
                      </div>
                    </FormItem>
                  )}
